      ...
      builder: (context, snap) {
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, i) {
            ...
            // Extract photos list: [{label, path}, ...]
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, i) {
            ...
            return _CompactTile(
              ...
              subtitle:
                  'Truck: ${truckId.isEmpty ? '—' : truckId} • ${status.isEmpty ? '—' : status.toUpperCase()} • $dateStr',
              trailing: Container(
      ...
      builder: (context, snap) {
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, i) {
            ...
            final status = (data['status'] ?? '').toString();
            final desc =
                (data['issueDescription'] ?? data['description'] ?? '').toString();
            ...
            return _CompactTile(
              ...
              subtitle:
                  '${desc.isEmpty ? '—' : desc} • ${status.isEmpty ? '—' : status.toUpperCase()} • $dateStr',
              trailing: Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                decoration: BoxDecoration(
                  color: statusColor.withValues(alpha: 0.12),
...
/// Displays an inspection photo from either a Firebase Storage URL
/// or a base64 data URI (data:image/jpeg;base64,...).
...
Color _maintenanceStatusColor(BuildContext context, String statusRaw) {
  ...
  if (s.contains('completed') ||
      s.contains('pickedup') ||
      s.contains('picked_up')) return Theme.of(context).colorScheme.tertiary;
  if (s.contains('urgent') || s.contains('unsafe')) {
    return Theme.of(context).colorScheme.error;
  }
  if (s.contains('approved') ||
      s.contains('scheduled') ||
      s.contains('service')) return Theme.of(context).colorScheme.secondary;
